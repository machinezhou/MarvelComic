...
public class SpaceLayoutDecoration extends RecyclerView.ItemDecoration {
  ...
  @Override public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
    ...
    //todo resolve conflicts of layoutViewType and layoutVertical etc.
    layoutViewType(outRect, itemViewType);
  }

  private void layoutViewType(Rect outRect, int t) {
    if (t == viewType && typeRect != null) {
      outRect.left = typeRect.left;
      outRect.top = typeRect.top;
      outRect.right = typeRect.right;
      outRect.bottom = typeRect.bottom;
    }
  }
  ...
  private void layoutVertical(Rect outRect, int position, int lastPosition) {
    ...
    outRect.left = leftCut == -1 ? 0 : leftCut;
    outRect.right = rightCut == -1 ? 0 : rightCut;
  ...
  private void layoutHorizontal(Rect outRect, int position, int lastPosition) {
    ...
    outRect.bottom = leftCut == -1 ? 0 : leftCut;
    outRect.top = rightCut == -1 ? 0 : rightCut;
    if (headCut != -1 && position == 0) {
  ...
  public static class Builder {
    ...
    /**
     * set decoration to specific type of child view
     */
    public Builder viewType(int viewType, Rect rect) {
      this.viewType = viewType;
      this.typeRect = rect;
      return this;
    }
    ...
    public SpaceLayoutDecoration build() {
      ...
      if ((viewType != DEFAULT_VIEW_TYPE && typeRect == null) || (viewType == DEFAULT_VIEW_TYPE
          && typeRect != null)) {
        Log.d(TAG, "view type or rect is missing");
      }
      return new SpaceLayoutDecoration(type, span, linearSpace, gridSpace, headCut, tailCut,
          leftCut, rightCut, rowSpace, columnSpace, viewType, typeRect);
    }
  }
}
